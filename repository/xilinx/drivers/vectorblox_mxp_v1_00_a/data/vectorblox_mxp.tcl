...
proc generate {drv_handle} {
    ...
    foreach periph $periphs {
        if { $vivado_version <= 2013.4 } {
		  ...
		  puts $file_handle "/* VCI_UIDs */"
		  set vci_interfaces [get_intf_pins -of_objects $periph M_VCI* ]
		  set current_vci 0
		  for {set vci 0} {$vci < [llength $vci_interfaces] } {incr vci} {
				set intf [::hsi::utils::get_connected_intf $periph M_VCI[set vci]]
				set num_functions [get_property "CONFIG.VCI_${vci}_FUNCTIONS" $periph]
				set cell [::hsi::get_cells -of_objects $intf]
				#			 puts $file_handle "//cell=$cell"
				set uid [get_property "CONFIG.VCI_UID" $cell]
				for { set f 0} { $f < $num_functions } {incr f} {
					 puts $file_handle "#define XPAR_VECTORBLOX_MXP_0_VCI_${current_vci}_UID $uid"
					 incr current_vci
				}
		  }
		  while { $current_vci < 16 } {
				puts $file_handle "#define XPAR_VECTORBLOX_MXP_0_VCI_${current_vci}_UID 0xFFFFFFFF"
				incr current_vci
		  }
		  #for {set vci 0} {$vci < 10} {incr vci} {
		  #		puts $file_handle "//periph=$periph"
		  #		set intf [::hsi::utils::get_connected_intf $periph M_VCI[set vci]]
		  #		if { $intf } {
		  #			 puts $file_handle "//intf= $intf"
		  #			 puts $file_handle "//properties = [list_property $intf]"
		  #			 set cell [::hsi::get_cells -of_objects $intf]
		  #			 puts $file_handle "//cell=$cell"
		  #			 set uid [get_property "CONFIG.VCI_UID" $cell]
		  #			 puts $file_handle "//uid = $uid"
		  #		} else{
		  #			 set uid -1
		  #		}
		  #			 puts $file_handle "#define XPAR_VECTORBLOX_MXP_0_VCI_${vci}_UID $uid"
		  #
		  #}
